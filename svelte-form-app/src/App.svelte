<script>
    ...
    let invoiceUrl = ''; // To store the invoice URL
    let isSubmitting = false; // To manage form submission state
    ...
    async function submitForm() {
        ...
        // Logic to handle form submission
         ...
         try {
            isSubmitting = true; // Set form submission state to true

            const response = await fetch('your_process_booking_endpoint', {
                method: 'POST',
                headers: {
            ...
            if (data && data.url) {
                ...
                console.error('Error fetching invoice URL');
                // Handle error
            }
        } catch (error) {
            ...
            isSubmitting = false; // Reset form submission state
        }
    }
...
{:else}
    <div class="min-h-screen bg-gray-700 flex items-center justify-center">
     ...
     <!-- calculator goes here -->
     <div class="bg-gray-800 border-2 border-gray-900 shadow-2xl rounded-lg grid grid-cols-4 gap-2 h-fit">
        <!-- form goes here -->
        ...
        <button class="col-span-1 bg-gray-500 hover:bg-gray-400 rounded-full p-5 text-white text-center"  on:click={submitForm} disabled={isSubmitting}>
            {#if isSubmitting}
                Processing...
            {:else}
                Next
