...
function process_booking($request) {
    ...
    // Connect to the database
		try {
			...
			$stmt->bindParam(':additionalLocations', json_encode($additionalLocations)); // Assuming this is an array or object
	...
	// Return a success response
		return new WP_REST_Response('Records inserted successfully.', 200);
		finally {
		// Close connection
			unset($pdo);
			}
// ... [Previous code to insert booking record] ...
  ...
  if ($executionResult) {
    ...
    $customer_id = get_square_customer_id($email); // A function that retrieves or creates a customer in Square and returns the customer ID.

    $orderData = create_square_order($params);

    // Step 1: Create an order in Square
    $orderResponse = create_square_order($orderData);
    ...
    if (isset($orderResponse['order']['id'])) {
        ...
        if (isset($invoiceResponse['invoice']['id'])) {
            ...
            if (isset($publishResponse['invoice'])) {
                return new WP_REST_Response("Invoice published successfully.", 200);
            } else {
                return new WP_REST_Response("Error in publishing invoice: " . json_encode($publishResponse), 500);
            }
        } else {
...
function create_square_order($params) {
    $accessToken = 'EAAAl26Nzs6BE1hAok0GyqLJYFJ8asO95pmgxJE4-_iq886L9jzhZi7iuEmTZ8d7'; // Replace with your Square access token
    $locationId = 'LBZ6V29STVG69'; // Replace with your Square location ID
...
function create_square_invoice($order_id, $customer_id) {
    ...
    $invoice_data = array(
        ...
        "invoice" => array(
            "order_id" => $order_id,
            "location_id" => "LBZ6V29STVG69", // Replace with your location ID
            ...
            "payment_requests" => array(
                array(
                    ...
                    "due_date" => "YYYY-MM-DD", // Set the due date
    ...
    $response = wp_remote_post($api_url, array(
        'headers' => array('Authorization' => 'Bearer YOUR_ACCESS_TOKEN', 'Content-Type' => 'application/json'),
...
function publish_square_invoice($invoice_id, $invoice_version) {
    ...
    if (empty($invoice_id) || empty($invoice_version)) {
    ...
    $response = wp_remote_post($api_url, array(
        'headers' => array(
            'Authorization' => 'Bearer EAAAl26Nzs6BE1hAok0GyqLJYFJ8asO95pmgxJE4-_iq886L9jzhZi7iuEmTZ8d7', // Replace with your actual access token
            'Content-Type' => 'application/json'
        ),
...
function get_square_customer_id($email, $name = '') {
    ...
    $access_token = 'EAAAl26Nzs6BE1hAok0GyqLJYFJ8asO95pmgxJE4-_iq886L9jzhZi7iuEmTZ8d7'; // Replace with your actual Square access token
