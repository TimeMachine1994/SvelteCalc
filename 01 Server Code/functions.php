...
// Function to handle the logic of your custom endpoint
function process_booking($request) {
    global $wpdb;
    if (!class_exists('wpdb')) {
        return new WP_REST_Response('wpdb class not found', 500);
    }

    // Get parameters from the request
    $params = $request->get_json_params();

	// Extract and sanitize/validate all fields
	$livestreamAtFuneralHome = isset($params['livestreamAtFuneralHome']) ? $params['livestreamAtFuneralHome'] : null;
	$selectedPackage = sanitize_text_field($params['selectedPackage']);
	$additionalLocations = $params['additionalLocations']; // Further processing might be needed based on structure
	$livestreamDuration = intval($params['livestreamDuration']);
	$masterPrice = intval($params['masterPrice']);
	$urlFriendlyText = sanitize_text_field($params['urlFriendlyText']);


    // Sanitize and Validate the data
    $lovedOnesName = sanitize_text_field($params['lovedOnesName']);
    $livestreamDate = sanitize_text_field($params['livestreamDate']); // Further validation might be required
    $yourName = sanitize_text_field($params['yourName']);
    $email = sanitize_email($params['email']);
    $phoneNumber = sanitize_text_field($params['phoneNumber']); // Further validation might be required
    $secondAddress = sanitize_text_field($params['secondAddress']);
    $thirdAddress = sanitize_text_field($params['thirdAddress']);
    $selectedPackage = sanitize_text_field($params['selectedPackage']);
    $masterPrice = intval($params['masterPrice']);
    $totalCost = intval($params['totalCost']);

    // Additional validation checks (e.g., email validation)
    if (!is_email($email)) {
    ...
    // Connect to the database
		try {
			...
			$sql = "INSERT INTO bookings (lovedOnesName, livestreamDate, yourName, email, phoneNumber, secondAddress, thirdAddress, selectedPackage, livestreamAtFuneralHome, additionalLocations, livestreamDuration, masterPrice, urlFriendlyText, totalCost) VALUES (:lovedOnesName, :livestreamDate, :yourName, :email, :phoneNumber, :secondAddress, :thirdAddress, :selectedPackage, :livestreamAtFuneralHome, :additionalLocations, :livestreamDuration, :masterPrice, :urlFriendlyText, :totalCost)";
			$stmt = $pdo->prepare($sql);

			// Bind parameters
			$stmt->bindParam(':lovedOnesName', $lovedOnesName);
			$stmt->bindParam(':livestreamDate', $livestreamDate);
			$stmt->bindParam(':yourName', $yourName);
			$stmt->bindParam(':email', $email);
			$stmt->bindParam(':phoneNumber', $phoneNumber);
			$stmt->bindParam(':secondAddress', $secondAddress);
			$stmt->bindParam(':thirdAddress', $thirdAddress);
			$stmt->bindParam(':selectedPackage', $selectedPackage);
			$stmt->bindParam(':livestreamAtFuneralHome', $livestreamAtFuneralHome);
			$stmt->bindParam(':additionalLocations', json_encode($additionalLocations));
			$stmt->bindParam(':livestreamDuration', $livestreamDuration, PDO::PARAM_INT);
			$stmt->bindParam(':masterPrice', $masterPrice, PDO::PARAM_INT);
			$stmt->bindParam(':urlFriendlyText', $urlFriendlyText);
			$stmt->bindParam(':totalCost', $totalCost, PDO::PARAM_INT);
			// Execute the prepared statement
			$executionResult = $stmt->execute();
		} catch(PDOException $e) {
			return new WP_REST_Response("Database error: " . $e->getMessage(), 500);
		} finally {
			// Close connection
			unset($pdo);
		}
    // Check if the booking record was inserted successfully
    if ($executionResult) {
        // Database insertion was successful

        // Define customer information for Square
        $customer_id = get_square_customer_id($email); // Implement this function to retrieve or create a customer in Square

      

        // Step 1: Create an order in Square
        $orderResponse = create_square_order($params);
        ...
        if (isset($orderResponse['order']['id'])) {
            ...
            $invoiceResponse = create_square_invoice($order_id, $customer_id);

            // Check if the invoice was successfully created
            if (isset($invoiceResponse['invoice']['id'])) {
                $invoice_id = $invoiceResponse['invoice']['id'];
                $invoice_version = $invoiceResponse['invoice']['version'];

                // Step 3: Publish the invoice
                $publishResponse = publish_square_invoice($invoice_id, $invoice_version);

              // Handle the publish response
				if (!str_starts_with($publishResponse, 'Error:')) {
					// The publish response is the URL
					$invoiceUrl = $publishResponse;
					// You can now return this URL in your response
					return new WP_REST_Response("Invoice published successfully. URL: " . $invoiceUrl, 200);
				} else {
					// Handle error
					return new WP_REST_Response($publishResponse, 500);
				}
            } else {
                return new WP_REST_Response("Error in creating invoice: " . json_encode($invoiceResponse), 500);
            }
        } else {
            return new WP_REST_Response("Error in creating order: " . json_encode($orderResponse), 500);
        }
    } else {
        return new WP_REST_Response("ERROR: Could not execute $sql. " . $stmt->errorInfo(), 500);
    }

    // Return a success response
    return new WP_REST_Response('Booking process completed successfully.', 200);
}
 
// Registering your custom endpoint
add_action('rest_api_init', function () {
    register_rest_route('book_now/v1', '/process_booking', array(
        'methods' => 'POST',
        'callback' => 'process_booking',
    ));
});


// Creating a square order using PHP based on user input
function create_square_order($params) {
    $accessToken = 'EAAAFH8ETJ9-ghYV5SdX67g83b57L7zMXyY_ayyde2TrnMKjrd4SvJgDqgAsST_y'; // Replace with your Square access token
    $locationId = 'FNN7E67QT9CP6'; // Replace with your Square location ID

    // Prepare line items array
    $lineItems = [];

    // Add selected package as a line item
    if ($params['selectedPackage']) {
        $lineItems[] = array(
            "name" => $params['selectedPackage'],
            "quantity" => "1",
            "base_price_money" => array(
                "amount" => $params['packagePrices'][$params['selectedPackage']] * 100, // Convert to cents
                "currency" => "USD"
            )
        );
    }
    ...
    if ($params['additionalLocations']['secondAddress']) {
        $lineItems[] = array(
            "name" => "Location B",
            "quantity" => "1",
            "base_price_money" => array(
                "amount" => 39900, // Assuming price is $399
                "currency" => "USD"
            )
        );
    }
    if ($params['additionalLocations']['thirdAddress']) {
        $lineItems[] = array(
            "name" => "Location C",
            "quantity" => "1",
            "base_price_money" => array(
                "amount" => 32900, // Assuming price is $329
                "currency" => "USD"
            )
        );
    }

    // Additional livestream duration
    if ($params['livestreamDuration'] > 2) {
        $extraHours = $params['livestreamDuration'] - 2;
        $lineItems[] = array(
            "name" => "Additional Livestream Time ({$extraHours}h)",
            "quantity" => "1",
            "base_price_money" => array(
                "amount" => $extraHours * 9900, // Assuming $99 per extra hour
                "currency" => "USD"
            )
        );
    }
    ...
    if (is_wp_error($response)) {
        ...
        return $data; // Contains the order ID and other details
    }
}

function create_square_invoice($order_id, $customer_id) {
    $api_url = 'https://connect.squareup.com/v2/invoices';
     // Calculate the due date as three days from now
	 $dueDate = new DateTime();
	 $dueDate->modify('+3 days');
	 $formattedDueDate = $dueDate->format('Y-m-d'); // Format the date as YYYY-MM-DD
 
    // Prepare the invoice data
    $invoice_data = array(
        "idempotency_key" => uniqid(), // Unique key for the request
        "invoice" => array(
            "order_id" => $order_id,
            "location_id" => "FNN7E67QT9CP6", // Replace with your actual location ID
            ...
            "payment_requests" => array(
                array(
                    ...
                    "due_date" => $formattedDueDate, // Correctly formatted future date
                    "tipping_enabled" => true
                )
            ),
            "delivery_method" => "EMAIL",
            "title" => "Livestream Service Invoice",
            "description" => "Invoice for Livestream Services",
            "accepted_payment_methods" => array(
                "card" => true // Accept card payments
            )
        )
    );

    // Send the request to Square
    $response = wp_remote_post($api_url, array(
        'headers' => array('Authorization' => 'Bearer EAAAFH8ETJ9-ghYV5SdX67g83b57L7zMXyY_ayyde2TrnMKjrd4SvJgDqgAsST_y', 'Content-Type' => 'application/json'), // Replace with your actual access token
...
function publish_square_invoice($invoice_id, $invoice_version) {
    // Ensure the function parameters are properly passed
    if (!isset($invoice_id) || $invoice_id === null || !isset($invoice_version) || $invoice_version === null) {
        return 'Error: Missing required parameters';
    }

    // Construct the API endpoint URL
    $api_url = "https://connect.squareup.com/v2/invoices/$invoice_id/publish";

    // Prepare the data to be sent in the request body
    $publish_data = array(
        "idempotency_key" => uniqid(), // Generate a unique key for the request
        "version" => $invoice_version
    );

    // Make the API request to publish the invoice
    $response = wp_remote_post($api_url, array(
        'headers' => array(
            'Authorization' => 'Bearer EAAAFH8ETJ9-ghYV5SdX67g83b57L7zMXyY_ayyde2TrnMKjrd4SvJgDqgAsST_y', // Replace with your actual access token
            'Content-Type' => 'application/json'
        ),
    ...
    return json_decode(wp_remote_retrieve_body($response), true);
	 ...
	 // Check if the invoice was published successfully and contains a public URL
	 if (isset($responseBody['invoice']) && isset($responseBody['invoice']['public_url'])) {
        return $responseBody['invoice']['public_url'];
    } else {
        return 'Error: Unable to retrieve invoice URL';
    }
}


function get_square_customer_id($email, $name = '') {
    $api_url = 'https://connect.squareup.com/v2/customers/search';
    $access_token = 'EAAAFH8ETJ9-ghYV5SdX67g83b57L7zMXyY_ayyde2TrnMKjrd4SvJgDqgAsST_y'; // Replace with your actual Square access token
    ...
    if (is_wp_error($response)) {
        // Handle error; could log or return an error message
        return null;
    }
    ...
    if (!empty($customers)) {
        // Return the first customer's ID
    ...
    $customer_data = [
        'given_name' => $name, // Assuming you have a name, split it into 'given_name' and 'family_name' if needed
        'email_address' => $email
